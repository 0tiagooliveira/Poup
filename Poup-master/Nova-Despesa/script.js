document.addEventListener('DOMContentLoaded', function() {
    ...
    const categoriasPadrao = [
        ...
        { nome: 'Vestuário', icone: 'checkroom' },
        { nome: 'Cuidados Pessoais', icone: 'face' },
        { nome: 'Pets', icone: 'pets' },
        { nome: 'Viagem', icone: 'flight' },
        { nome: 'Presentes', icone: 'celebration' },
        { nome: 'Doações', icone: 'volunteer_activism' },
        { nome: 'Impostos', icone: 'account_balance' },
        { nome: 'Seguros', icone: 'security' },
        { nome: 'Cartão de Crédito', icone: 'credit_card' },
        { nome: 'Empréstimos', icone: 'account_balance_wallet' },
        { nome: 'Investimentos', icone: 'trending_up' },
        { nome: 'Negócios', icone: 'business' },
        { nome: 'Manutenção', icone: 'build' },
        { nome: 'Combustível', icone: 'local_gas_station' },
        { nome: 'Telecomunicações', icone: 'phone' },
        { nome: 'Contas de Casa', icone: 'electric_bolt' },
        { nome: 'Família', icone: 'group' },
        { nome: 'Cuidados com Idosos', icone: 'elderly' },
        { nome: 'Cuidados com Crianças', icone: 'child_care' },
    ...
    function salvarDespesa() {
        ...
        if (categoriaElemento) {
            const iconeElemento = categoriaElemento.querySelector('.material-icons');
            ...
            if (estiloFundo) {
                corCategoria = estiloFundo.style.backgroundColor || 
                             getComputedStyle(estiloFundo).backgroundColor;
            }
        }
        ...
        const novaDespesa = {
            ...
            iconeCategoria: iconeCategoria, // NOVO: Salvar ícone da categoria
        ...
        Promise.all([
            ...
            // Criar notificação da despesa
            criarNotificacaoDespesa(novaDespesa);
    ...
    function salvarLocalStorage(despesa) {
        return new Promise((resolve) => {
            try {
                ...
                atualizarSaldoContaLocalStorage(despesa);
    ...
    function atualizarSaldoContaLocalStorage(despesa) {
        try {
            ...
            // Converter valor da despesa para número
            ...
            if (isNaN(valorDespesa)) {
                console.error('Valor da despesa inválido:', despesa.valor);
                return;
            }
            ...
            if (contaIndex !== -1) {
                ...
                // Calcular novo saldo (subtraindo despesa)
                const novoSaldo = saldoAtual - valorDespesa;
                
                console.log(`Atualizando saldo localStorage: ${saldoAtual} - ${valorDespesa} = ${novoSaldo}`);
        ...
        return new Promise((resolve, reject) => {
            ...
            firebase.firestore().collection('despesas').add(despesaFirestore)
                .then(() => {
                    ...
                    // Atualizar saldo da conta após salvar despesa
                    return atualizarSaldoConta(despesa);
                })
                .then(() => {
                    resolve();
                })
    ...
    function atualizarSaldoConta(despesa) {
        return new Promise((resolve, reject) => {
            ...
            firebase.firestore().collection('contas')
                ...
                .then(snapshot => {
                    ...
                    const novoSaldo = saldoAtual - valorDespesa; // Subtraindo despesa

                    console.log(`Atualizando saldo: ${saldoAtual} - ${valorDespesa} = ${novoSaldo}`);
...
// Função para criar notificação de despesa
function criarNotificacaoDespesa(despesa) {
    try {
        ...
        if (window.parent && window.parent.notificacoesManager) {
            ...
            window.parent.notificacoesManager.notificarDespesa(despesa);
            return;
        }
        
        if (window.opener && window.opener.notificacoesManager) {
            console.log('Enviando notificação via window.opener');
            window.opener.notificacoesManager.notificarDespesa(despesa);
            return;
        }
