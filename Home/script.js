...
async function inicializarComponentes(user) {
    ...
    // Carregar nome e avatar das configurações
...
// Função para carregar dados do perfil (nome e avatar) salvos nas Configurações
...
// Carregar notificações para todas as receitas e despesas existentes
async function carregarNotificacoesTransacoes(userId) {
    ...
    try {
        ...
        // Carregar receitas do mês atual
        ...
        receitasSnapshot.forEach(doc => {
            ...
            if (isDataNoMesSelecionado(receita.data, mesAtual, anoAtual)) {
                ...
                const status = receita.recebido || receita.pago || receita.concluida ? '✅' : '⏳';
        ...
        despesasSnapshot.forEach(doc => {
            ...
            if (isDataNoMesSelecionado(despesa.data, mesAtual, anoAtual)) {
                ...
                const status = despesa.pago || despesa.concluida || despesa.recebido ? '✅' : '⏳';
        ...
        // Marcar como carregadas nesta sessão
        sessionStorage.setItem('notificacoesCarregadas', 'true');
        
        console.log(`✅ ${totalNotificacoes} notificações de transações carregadas`);
    } catch (error) {
        console.error('❌ Erro ao carregar notificações de transações:', error);
    }
}
...
function renderizarCartoesCredito(cartoes) {
    ...
    if (cartoes.length > 0) {
        ...
        cartoes.slice(0, 3).forEach(cartao => {
            ...
            // Função para obter o ícone e cor do banco
            function obterDadosBanco(nomeBanco) {
                const banco = (nomeBanco || '').toLowerCase();
                if (banco.includes('picpay')) return {
                    icone: '../Icon/picpay.svg',
                    cor: 'var(--picpay, #21C25E)',
                    nome: 'PicPay'
                };
                if (banco.includes('itau') || banco.includes('itaú')) return {
                    icone: '../Icon/itau.svg',
                    cor: 'var(--itau, #EC7000)',
                    nome: 'Itaú'
                };
                if (banco.includes('nubank')) return {
                    icone: '../Icon/Nubank.svg',
                    cor: 'var(--nubank, #820ad1)',
                    nome: 'Nubank'
                };
                if (banco.includes('bradesco')) return {
                    icone: '../Icon/bradesco.svg',
                    cor: 'var(--bradesco, #CC092F)',
                    nome: 'Bradesco'
                };
                if (banco.includes('santander')) return {
                    icone: '../Icon/santander.svg',
                    cor: 'var(--santander, #EC0000)',
                    nome: 'Santander'
                };
                if (banco.includes('caixa')) return {
                    icone: '../Icon/caixa.svg',
                    cor: 'var(--caixa, #0070AF)',
                    nome: 'Caixa'
                };
                if (banco.includes('banco do brasil') || banco.includes('bb')) return {
                    icone: '../Icon/banco-do-brasil.svg',
                    cor: 'var(--banco-brasil, #FFEF38)',
                    nome: 'Banco do Brasil'
                };
                return {
                    icone: '../Icon/cartao-credito.svg',
                    cor: 'var(--outros, #6B7280)',
                    nome: cartao.nomeBanco || cartao.nomeCartao || 'Cartão'
                };
            }
            
            const dadosBanco = obterDadosBanco(cartao.nomeBanco || cartao.nomeCartao);
            ...
            const cartaoHTML = `
                <div class="cartao-credito-home" onclick="window.location.href='../Lista-de-cartoes/Lista-de-cartoes.html'">
                    <div class="cartao-credito-header">
                        <div class="conta-ux-icone conta-ux-icone-svg" data-color="${dadosBanco.cor}" style="background:${dadosBanco.cor};border-radius:50%;width:54px;height:54px;display:flex;align-items:center;justify-content:center;margin-right:16px;">
                            <img src="${dadosBanco.icone}" alt="${dadosBanco.nome}" style="width:32px;height:32px;object-fit:contain;">
                        </div>
...
function inicializarNotificacoes() {
    if (usuario && !notificacoesManager) {
        ...
        // Carregar notificações de transações existentes
        setTimeout(() => {
        ...
        // DESABILITADO: Notificações de resumo (apenas as transações individuais)
        // setTimeout(() => {
